...
public class ImagesHelper : IImageHelper
{
    ...
    public ImagesHelper(IWebHostEnvironment webHostEnvironment)
    {
    ...
    public async Task<ImageUploadedDto> Upload(string name, IFormFile imageFile,ImageType imageType, string folderName = null)
    {
        ...
        string newFileName = $"{name}_{dateTime.Microsecond}{fileExtention}";
        ...
        string message = imageType == ImageType.User
            ? $"{newFileName} isimli kullanici resmi basari ile ekelnmistir"
            : $"{newFileName} isimli makaler resmi basari ile eklemmistir";
